...
function getData(theData, station) {
	for (i = 0; i < theData.data.length; i++) {
		...
		var direction = theData.data.attributes.direction_id;
		if (direction == 0) {
			station.arrival.push("Southbound (Ashmont/Braintree)");
		}
		else {
			station.arrival.push("Northbound (Alewife)");
		}
	}
...
function closestDistance(loc, stations) {
	...
	var my_infowindow = new google.maps.InfoWindow({
		content: "<h1>Current Location</h1>" + 
			"<div>Closest Station: " + min_loc.s_name + "</div>"
			+ "<div>Distance: " + minimum + " miles</div>"
	});
